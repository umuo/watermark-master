...
- **高级优雅的界面设计**：全局融入 Glassmorphism（玻璃拟态）质感、现代平滑渐变底色和精美的悬浮交互动画
- **完美的移动端适配**：表单面板采用自动适应设备的紧凑 Grid 网格排版，小屏幕不再拥挤，单手操作畅快无阻
- **局域离线运行**：样式与基础处理代码皆内置。特殊针对苹果设备的 HEIC/HEIF 转换会智能利用优质网络 CDN 节点最新解码器，其余动作仍在用户本地运转，大幅保证隐私及安全！
- **支持苹果 HEIC 格式**：自动在客户端将 iOS 设备的 HEIC/HEIF 照片转换为支持水印处理的格式
- 支持批量处理多张图片（最多20张）
- 自定义水印文字
  - 支持多行文字（最多3行）
  - 支持复用上一次使用的水印文字
